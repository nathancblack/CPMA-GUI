...
def generate_config_content(settings_groups: List[Dict], export: bool) -> str:
    ...
    if export:
        lines = ["// Exported From CPMA GUI Config Editor"]
...
def save_current_config(cfg_path: Path, autoexec_path: Path, exported: bool):
    cfg_path = Path(cfg_path)
    if autoexec_path:
        ensure_autoexec_reference(autoexec_path)
    if exported:
        file_content = generate_config_content(ALL_SETTINGS_GROUPS, True)

    else:
        file_content = generate_config_content(ALL_SETTINGS_GROUPS, False)
